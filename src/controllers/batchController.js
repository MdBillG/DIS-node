...
const BatchController = {
  ...
  removeStudentFromBatch: async (req, res) => {
    try {
      const { batchId, studentId } = req.body;
      const batch = await BatchService.removeStudentFromBatch(
        batchId,
        studentId
      );
  ...
  removeTeacherFromBatch: async (req, res) => {
    try {
      const { batchId, teacherId } = req.body;
      const batch = await BatchService.removeTeacherFromBatch(
        batchId,
        teacherId
      );
      res.status(200).json({ success: true, batch });
  ...
  getAllStudentsByBatchAndTeacher: async (req, res) => {
    try {
      ...
      const students = await BatchService.getStudentsByBatchAndTeacher(
        batchId,
        teacherId
      );
